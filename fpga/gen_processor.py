...
if mode == "classic":
    test_mode = 0
    key_depth = V*(V+1)//2 + 2*V*O + O*(O+1)//2 + 1
elif mode == "pkc":
    test_mode = 1
    key_depth = V*O + V*(V+1)//2 + O*(O+1)//2 + 1
elif mode == "pkc_skc":
    test_mode = 2
    key_depth = V*rand_state_num + V*(V+1)//2 + O*(O+1)//2 + 1

key_addr_len = math.ceil(math.log(key_depth, 2))
if (math.ceil(math.log(key_depth, 2)) < 2*math.ceil(math.log(matrix_sz+1, 2))):
...
if key_depth > 1024:
    verilog_module += """
module bram16Kx"""+str(bram16k_num)+""" #(
    parameter WIDTH = 8,
    parameter DEPTH = 9,
    parameter FILE = "",
    parameter DELAY = 0
    ...
    for i in range(bram16k_num):
        if i == bram16k_num-1:
            if ((i+1)*O*GF_bit/32) >= platform_bram*2:
                ram_type = "lutmem"
    ...
    """+ram_type+""" #(
        ...
        .wren ((tmp_wren && (tmp_wdaddress[$clog2(DEPTH)-1:9] == """+str(i)+"""))),
        .q (q"""+str(i)+""")
    );
        """
        else:
...
bram_type = "mem"
if key_depth > 1024:
    bram_type = "bram16Kx"+str(bram16k_num) 
...
module tile_{r_idx}_{c_idx}#(
    ...
    generate
        if (TILE_COL_IDX < NUM_PROC_COL) begin: key_mem
            ...
            {bram_type} #(
                ...
                .clock (clk),
                .data (key_write_data),
                .rdaddress (op_in[$clog2({key_depth})-1:0]),
                .wraddress (op_in[$clog2({key_depth})-1:0]),
                .wren (key_wren),
                .q (key_data)
            );
            for (j = 0; j < ROW; j=j+1) begin
                assign key_data_w[j][0] = (j < {o_width}) ? key_data[j*GF_BIT+:GF_BIT] : 0; // load from
    ...
    //         $display("| dataB_in: %x, addr: %d, key_data: %x, key_write_data: %x, key_wren: %d", dataB_in, op_in[$clog2({key_depth})-1:0], key_data, key_write_data, key_wren);
    ...
    // end
        """.format(r_idx=r_idx, c_idx=c_idx, r_num=r_num, c_num=c_num, GF_bit=GF_bit,
            ...
            key_depth=key_depth, matrix_sz=matrix_sz, tile_r_idx=r_idx, tile_c_idx=c_idx, 
            num_proc_col=O_, bram_type=bram_type, op_code_len=op_code_len, o_width=o_width)
    ...
    processor_{ab_type} #(
        ...
        .functionA(functionA_dup)
                    ...
                    """.format(r_idx=i, c_idx=j, current_row=current_row, current_col=current_col,
                               tile_r_idx=r_idx, tile_c_idx=c_idx, num_proc_col=O_,
                               op_code_len=op_code_len_proc, ab_type=ab_type)
                else:
                    b_type = "B"
                    op_code_len_proc = 3
                    if c_idx == 0 and j == 0:
    ...
    processor_{b_type} #(
        ...
        .r(r_w[{r_idx}][{c_idx}])
                    ...
                    """.format(r_idx=i, c_idx=j, current_row=current_row, current_col=current_col, 
                               tile_r_idx=r_idx, tile_c_idx=c_idx, num_proc_col=O_,
                               op_code_len=op_code_len_proc, b_type=b_type)
...
with open("./rtl/matrix.v", "w") as sources:
    sources.write(verilog_module)

### modify delay information
with open("./rtl/uov.v", "r") as sources:
...
line_all = ""
with open("./rtl/define.v", "r") as sources:
    lines = sources.readlines()
    for line in lines:
        line_all += line

with open("./rtl/define.v", "w") as sources:
...
simulator_arguments = "python simulator/codegen.py -n {matrix_sz} -v {V} -o {O} -g {GF_bit} -m {mode} -r {aes_round}".format(
