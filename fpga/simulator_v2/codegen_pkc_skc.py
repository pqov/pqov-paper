...
def sign_codegen():
    ...
    #
    # Process this way:
    # Generate 4 columns of P2.
    # Then, do (v^T P2) to prepare L
    #
    #              _ r4: Actual column of P2
    #             |  _ r1: start of the sliced column
    # P2:         | |   
    ...
    if use_inversion:
        ...
        addi(r2, r0, 0)                                                     # _(Loop r2)______________   | |
        addi(r3, r8, 0)                                                     # r3 = r8                 \  | |
        aes_set_ctr(r0, r3, P2_start_ctr)                                   # genkey before            | | |
        if O == 44:                                                         # Ip, special case         | | |
            aes_update_ctr((2<<16)|(O*rand_state_num//N+1))                 # Generate 8*imm bytes     | | |
            if use_pipelined_aes:                                           #                          | | |
                stall(aes_round+3)                                          # Stall for O bytes        | | |
        ...
        if O == 44:                                                         # of aes128ctr, |        | | | |
            ...
            aes_update_ctr(O*rand_state_num//N)                             # _____________/         | | | |
        mul_key_o_pipe(P1, r2, r0, O1, r0, r4)                              # Calculate L[r2][r5]    | | | |
        store_keys(L, r2, r5)                                               # Store to L[r2][r5]     | | | |
        addi(r4, r4, 1)                                                     # r4++                   | | | |
        addi(r5, r5, 1)                                                     # r5++                   | | | |
        bne(r5, r12, gen_rand_col_and_calculate)                            # ______________________/  | | |
        ...
        addi(r2, r1, 0)                                                     # r2: Actual column of L     | |
        addi(r3, r0, 0)                                                     # _(Loop r3)_____________    | |
        eval_L = len(inst)                                                  #                        \   | |
        load_keys(ZERO, r0, r0)                                             # Load zeroes             |  | |
        calc_l(L, r0, r3)                                                   # Calculate a column of L |  | |
        store_l(r2)                                                         # Store to column r2 of L |  | |
        stall(max((O - (math.ceil(V/bank)+N+5)), 0))                        # wait for transpose      |  | |
        addi(r2, r2, 1)                                                     # r2++                    |  | |
        addi(r3, r3, 1)                                                     # r3++                    |  | |
        bne(r3, r12, eval_L)                                                # _______________________/   | |

        addi(r1, r1, rand_state_num)                                        # r1+=4                      | |
        addi(r8, r8, O*rand_state_num//N)                                   # Increment r8               | |
        bne(r1, r15, gen_rand_outer_col)                                    # __________________________/  |
...
def vrfy_codegen():
    ...
    if not use_pipelined_aes:                                               #
        stall(max((P1_stall-rand_state_num*O_)*buffer_multiplier+20-(O*(O+1)//2//5), 0)) # Stall for AES or eval
