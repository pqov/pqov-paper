...
int sk_to_pk( pk_t * rpk , const sk_t* sk , const unsigned char *pk_seed)
{
    ...
    prng_gen_publicinputs(&prng1, rpk_P2 , sizeof(sk->L) );

    // P3
    unsigned char * rpk_P3 = rpk_P2 + sizeof(sk->L);
    calculate_P3( rpk_P3 , sk->P1 , rpk_P2 , sk->t1 );
...
int generate_keypair( pk_t * rpk, sk_t* sk, const unsigned char *pk_seed, const unsigned char *sk_seed )
{
    ...
    hash_final_digest( sk->t1 , sizeof(sk->t1) , &hctx );

    // prng for pk
    prng_publicinputs_t prng1;
    prng_set_publicinputs(&prng1 , pk_seed );
    // P1 and P2
    prng_gen_publicinputs(&prng1, rpk->pk , sizeof(sk->P1)+sizeof(sk->L) );
    memcpy( sk->P1 , rpk->pk , sizeof(sk->P1) );

    // L and P3
    unsigned char * rpk_P2 = rpk->pk + sizeof(sk->P1);
    unsigned char * rpk_P3 = rpk->pk + sizeof(sk->P1) + sizeof(sk->L);
    calculate_F2_P3( sk->L , rpk_P3 , sk->P1 , rpk_P2 , sk->t1 );
    return 0;
}
...
int expand_pk_predicate( pk_t * rpk , const cpk_t * cpk , const unsigned char *predicate)
{
    ...
    for(unsigned i=0;i<_V;i++) {
        ...
        if( 0 == predicate[i] ) { prng_skip_publicinputs(&prng0 , len_i ); }
        else                    { prng_gen_publicinputs(&prng0 , p1p2 , len_i ); }
        p1p2 += len_i;
    }
    // P2
    for(unsigned i=0;i<_V;i++) {
