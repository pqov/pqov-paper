      ...
      unsigned num_gfele_x , unsigned num_vinegar , unsigned vec_len )
{
...
#endif
      if( !_x[i] ) { trimat += vec_len*(v-i); continue; }
      for(unsigned j=i;j<v;j++) _xixj[j]=_x[j];
   ...
   // P2
   for(unsigned i=0;i<v;i++) {
      if( !_x[i] ) { trimat += vec_len*o; continue; }
      for(unsigned j=0;j<o;j++) _xixj[j]=_x[v+j];
   ...
   for(unsigned i=0;i<o;i++) {
      ...
      unsigned i_start = i-(i&3);
#endif
      if( !_x[v+i] ) { trimat += vec_len*(o-i); continue; }
      for(unsigned j=i;j<o;j++) _xixj[j]=_x[v+j];
...
void accu_eval_quad_gf16( unsigned char * accu_res , const unsigned char * trimat , const unsigned char * x_in_byte , unsigned num_gfele_x , unsigned vec_len )
{
   ...
   for(unsigned i=0;i<n;i++) {
      ...
      unsigned i_start = i-(i&3);
#endif
      if( !_x[i] ) { trimat += vec_len*(n-i); continue; }
